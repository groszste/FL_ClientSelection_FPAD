...
cs_algorithm = "PowerOfChoice"

#load data files
data_dir = 'exp2_siwm_print_at_server' #(exp2_siwm_replay_at_server, exp2_siwm_print_at_server)
classes = {'Live': 0, 'PA': 1}
server_model_path = data_dir + '/results/server/round_4'
...
if (cs_algorithm == 'FedAvg'):
	...
	for comm_round in range(comms_round):
		...
		with open(data_dir + '/results/' + cs_algorithm + '/results.txt', 'a+') as f:
			f.write('ROUND ' + str(comm_round) + ': SIWM_Print_Acc={:.4} SIWM_Replay_Acc={:.4} SIWM_Print_Loss={:.4} SIWM_Replay_Loss={:.4} WD={:.4} '.\
				format(acc_siwm_print, acc_siwm_replay, loss_siwm_print, loss_siwm_replay, np.mean(wd)) + 'Clients=' + ','.join(client_names) + '\n')
	...
	for comm_round in range(comms_round):
		...
		with open(data_dir + '/results/' + cs_algorithm + '/results.txt', 'a+') as f:
			f.write('ROUND ' + str(comm_round) + ': SIWM_Print_Acc={:.4} SIWM_Replay_Acc={:.4} SIWM_Print_Loss={:.4} SIWM_Replay_Loss={:.4} WD={:.4} '.\
				format(acc_siwm_print, acc_siwm_replay, loss_siwm_print, loss_siwm_replay, np.mean(clients_weight_divergence)) + \
